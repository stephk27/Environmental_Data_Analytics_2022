...
water_df <- data.frame(
  ...
  "PWSID" = rep(pwsid, 12),
  "Average_Use" = round(as.numeric(avg.use.mgd), 2)) %>%
  arrange(Date)


#5
ggplot(water_df, aes(x = Date, y = Average_Use)) +
  geom_point() +
  labs(title = "Durham Monthly Water Use (2019)", x = "Date", y = "Average Water Use (MGD)")
...
scrape.it <- function(PWSID, year){
  ...
  #Get the proper webpage
    webpage <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', PWSID, '&year=', year))
