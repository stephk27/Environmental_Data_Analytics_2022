...
> Answer:
H0: The mean lake temperature at depth in July = 0 
Ha: The mean != 0
...
NTL_LTER_tidy <- NTL_LTER %>%
  ...
  na.omit() #check if this is how I do the na.omit or should I be using is.na?
...
> Answer: The figure suggests that temperatures generally decrease with a lower subsurface depth.  While there is a general downward sloping trend, the temperatures vary significantly with each depth measurement and the results do not show a linear trend.
...
#add residual plots & other tests of fit
```
...
```{r anova.model}
#Do we need to include all of the lead up tests that are in the labs? Do we need to group by lakename first? And should this incorporate depth?
#12
laketemp <- NTL_LTER %>%
  group_by(lakename, temperature_C, depth) %>%
  summarize(temperature_C = sum(temperature_C))

#laketemp_lm <- lm(data = NTL_LTER_tidy, temperature_C ~ lakename)
laketemp_lm <- lm(data = laketemp, temperature_C ~ lakename)
summary(laketemp_lm)

#laketemp_anova <- aov(data = NTL_LTER_tidy, temperature_C ~ lakename)
laketemp_anova <- aov(data = laketemp, temperature_C ~ lakename)
...
> Answer: The results show that we can reject the null hypothesis and that there is a statistically significant (p-value <= .05) difference in means between temperatures across the lakes. The lm and aov models report a similar p-value of approximately <2e-16.
...
>Answer: Crampton Lake and Central Long Lake, and Ward Lake and Hummingbird Lake have very similar mean temperatures statistically because the p-values are greater than 0.05 and the difference in means is less than 1.  ##I need help with the second part of the question.
