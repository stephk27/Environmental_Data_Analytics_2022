...
shapiro.test((EPAair.subsample$Ozone)) #evaluate normal distribution
...
#reject H0 and accept alternative, means are not the same
...
#as.factor - variable needs to be categorical to separate b/c it was an integer

ggplot(EPAair, aes(x = as.factor(Year), y = Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, label = c("b", "a"))+ #add label (b, a) to max value
  labs(x = "", y = "Ozone")
...
#r squared is really low and therefore a problem
#other things explaining variability other than ozone
...
corrplot(EPAair.subset, method = "ellipse")
...
PM2.5_AIC <- lm(data = EPAair, PM2.5 ~ Month + Year + meanLat + meanLng + Ozone)
...
PM2.5_bestmodel <- lm(data = EPAair, PM2.5 ~ Month + meanLat + meanLng + Ozone)
summary(PM2.5_bestmodel)
