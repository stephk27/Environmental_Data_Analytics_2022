...
<<<<<<< HEAD
#setwd("/Users/lmm89/OneDrive/Duke_University/7_Spring2021/ENV872_EDA/GitRepo_EDA_S2021/Environmental_Data_Analytics_2022")
=======
...
> Answer: Output shows in the console.  It also shows all the packages that are included in the parent package.
...
#dec = decimal; useful for international data

```{r importdata}
# Absolute file path (not recommended)
<<<<<<< HEAD
#USGS.flow.data <- read.csv("C:/Users/skins/Documents/EDA/Environmental_Data_Analytics_2022/Data/Raw/USGS_Site02085000_Flow_Raw.csv")
...
str(USGS.flow.data)#shows all objects in dataframe
dim(USGS.flow.data) #shows number of rows & columns
length(USGS.flow.data) #number of columns

head(USGS.flow.data) #default is 6 first rows
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5) #shows last few rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #shows specific block of df; rows then columns

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) #provides summary stats for the columns

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: filled in as N/A
...
SK: Must be careful when we are removing NAs when they are in the middle of the dataset.  We won't be doing this in EDA.
...
> Answer: It can be helpful for quickly calculating summary stats.

```{r removeNA}
#how to remove NAs - you should create a new data set so that you have both versions
...
#will use lubridate in later classes
...
```{r fixformartDate}
# using format function transforms data into characters
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

#fixes format of early dates that are pre-2000 but R didn't recognize as such
  #here you use a greater than specific number to recognize the 1900s
  #d = input
...
#transforms data back into dates
...
<<<<<<< HEAD
write.csv(USGS.flow.data, file = "C:/Users/skins/Documents/EDA/Environmental_Data_Analytics_2022/Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE)
=======
