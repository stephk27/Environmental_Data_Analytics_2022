...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) #don't specify color within aes b/c then puts layer within plot
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? - ggplot does this for you
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + #shape assigns shape of point so assigns different shape for each Year, Color is a function of site name
  geom_point()
print(PMplot)
...
#create new plot for each facet = Site.Name
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), #only include subset to plot
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #site name by Year - year for columns and sites by rows - creates matrix
...
PMplot.line <- 
  ...
  geom_line()
print(PMplot.line)
#for line you need to make sure your x axis is continuous (makes sense over time)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#note indicators in the r chunk at the top
# Scatterplot
lightvsDO <- 
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + #says min and max for axes
  ylim(0, 20)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #because with limnology you are looking at depth into the ocean so want deeper to be below 0
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) + #color based on depth
  geom_point() +
  geom_smooth(method = lm) + #add trend line - lm = linear model
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - color is for contour of the shape 
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", #stacking observations over "y"
               binwidth = 1, 
               stackdir = "center", #where are you stacking - stack direction
               position = "dodge", #makes dots that are the same concentration go to the side rather than overlaying so you can see them all
               dotsize = 2) 
print(Nutrientplot4)
...
Nutrientplot6 <-
  ggplot(PeterPaul.chem.nutrients) + #diff x variable for each polygon and assign a different color
...
#problems:
  #no legend
  #annoying to use geom for each x

# Using a gathered dataset
#used spread/split dataset from before
Nutrientplot7 <-   
