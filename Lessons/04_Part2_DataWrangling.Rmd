...
#tells it which names it should use and assign it to a column (names_to) - nutrient name. values_to: values named as concentration

#remove rows with n/a in concentration column
#is.na() function asks if row has n/a value
...
#making dataset wider than original
...
#opposite transformation of pivot_longer
...
#data, column to be separated, how you want it to be separated
...
**must find what columns are in common
...
***short for row bind similar to cbind
...
***specify which files you want to read (.csv) within a folder
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID"))#common columns which will match when joining
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), #& means you get data without n/as for any inputs
