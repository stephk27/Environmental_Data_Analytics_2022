...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #Null is for columns based on space, month, average - null defines useless columns (space, average)
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
...
#merge together the two time series - want to make sure you have equally spaced observations
...
#summary will only show NAs if entire observation was NA
#you can run the summary function for each column to show NAs
...
>Answer: The data seems to show lines of seasonality because of the up-down trend.  The temperature also clearly increases over time, whereas the rainfall is constant.
...
#must call out column in ts function i.e. Rainfall column
fmonth <- month(first(BRA_complete$Date))
fyear <- year(first(BRA_complete$Date))

rainfall_ts <- ts(BRA_complete$Rainfall, start = c(fmonth, fyear), frequency = 12) #freq refers to repetition in dataset
