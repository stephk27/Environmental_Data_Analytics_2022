...
vector1
vector2 <- c("one","two","three") # character vector requires quotes
vector2
...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #fills column by column - default 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #change matrix to fill row by row
matrix2
...
matrix1[ , 3] #returns column 4
matrix1[c(12, 14)] # returns row 12 and column 14
matrix1[c(12:14)] #returns elements 12-14
matrix1[2:4, 1:3] #returns rows 2-4 and columns 1-3
...
list1[[2]]
#lists allow you to have many objects: matrix, arrays, vectors, etc.
#can also use $ to look at specific object
...
> Answer:The tabe brings up all elements under the dataframe name. With the arrow buttons you can select which element you want.
...
```{r}
temp_high_F <- c(49, 57, 39, 41, 44, 37, 40, 51, 52, 52)
temp_low_F <- c(27, 30, 24, 17, 26, 22, 18, 26, 26, 28)
```
...
temp_high_C <- C_conv(temp_high_F)

temp_low_C <- C_conv(temp_low_F)
...
```{r}
temp_df <- data.frame("High_F"=temp_high_F, "Low_F" = temp_low_F, "High_C" = temp_high_C, "Low_C" = temp_low_C)
...
```{r}
summary(temp_df)
sd(temp_high_F) #sd function can not be used for data frame. Must use one vector at a time
...
#Since the format is year-month-day we will use function ymd()*** - converts to date format
#doesn't matter how it's abbreviated or how it's written - all embedded in the function
