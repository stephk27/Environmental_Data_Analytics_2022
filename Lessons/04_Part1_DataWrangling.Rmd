...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #format is current format as character before turning into date
```
...
# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #filter(data to filter, criteria)
...
#%in% - function to filter by specific names

# Choose a range of conditions of a numeric or integer variable - selecting day number from June through October
#152 is first day of June
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) 
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma = combination of both
...
NTL.phys.data.TuesLake.90s <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 >= 1990 & year4 <= 1999)

```
Question: Why don't we filter using row numbers?

> Answer: Because then you need to unnecessarily scroll through your data to find the correct row numbers to use.
...
#East Long Lake in July 1998 has the highest temperature.
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #selects lakename & selects columns from sampledate to temp
...
#new column based on info from another column
...
```{r}
# add a month column to the dataset - can just use function month from lubridate
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #reorders columns based on order listed within select function

# find out the start and end dates of the dataset
#interval specifies start & end dates
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613])

#function first & last from dplyr
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate))
#returns first and last date based on sampledate
```
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #with pipe you don't need to reprovide name of dataset
...
>Answer: Keep original file but good to create new file for processed data. Good to have own R script for wrangling data so that R script for analysis is not ridiculously long.  Keep two separate files.
