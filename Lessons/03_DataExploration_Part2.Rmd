...
SK: Essential to understand structure, assumptions, trends, etc.
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y-%m-%d") #date should show how it is currently structured, not how you want to change it
class(USGS.flow.data$datetime)
...
SK: first think about how the data is measured and whether there could be biases
...
#ggplot - stands for "grammer of graphics"
ggplot((USGS.flow.data, aes(x = discharge.mean.approval) +
  geom_bar()
#x= which attribute the bar chart will show
...
#with Help function you can find default number of bins
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#geom_freqpoly
#second line adds in histogram from before
ggplot(USGS.flow.data) +
...
#lty = line type to make dashed, etc.

#colors the values by gage.height.mean.approval - will assign colors to each value
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#box plot - shows box within IQR &  outliers
#x & y shows two data sets
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#boxplot with slicing with group command
#cutwidth shows distribution of values within selected data - x & y on axes
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot: adds distribution and range of distribution within IQR through fatness
ggplot(USGS.flow.data) +
