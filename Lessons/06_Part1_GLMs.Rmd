...
**Single Linear Regression** Continuous response, one continuous explanatory variable.
  -will create linear relationship for everything even if there is no good correlation
...
alpha = intercept, beta = slope for straight line
E thing is error term
...
The goal for the linear regression is to find a **line of best fit**, which is the line drawn through the bivariate space that minimizes the total distance of points from the line. This is also called a "least squares" regression. The remainder of the variance not explained by the model is called the **residual error.** 
  -difference of point in regression line and real observation
  -want it to follow normal distribution
  -want a mean of 0 - ideal for residual error
  -always check residuals when you run a lm
...
Important components of the linear regression are the correlation and the R-squared value. The **correlation** is a number between -1 and 1, describing the relationship between the variables. Correlations close to -1 represent strong negative correlations, correlations close to zero represent weak correlations, and correlations close to 1 represent strong positive correlations. The **R-squared value** is the correlation squared, becoming a number between 0 and 1. The R-squared value describes the percent of variance accounted for by the explanatory variables.
  -r squared: tells how much of the variability in the model is explained by independent/explanatory variable
  -stronger correlation = 1
  -p value tells you significance of regression with respect to every coefficient; is it significantly different from 0?
      -if not significantly different, then not a good measure
      
Three things to look for in lm:
  -residuals
  -r squared
  -p value
  
If any of these things fail, then look at more variables.

## Simple Linear Regression

one single variable = simple

For the NTL-LTER dataset, can we predict irradiance (light level) from depth?

```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #y~x
...
>  ??We reject the null hypothesis and find that there is a negative correlation between irradiance and depth.
...
#looking for symmetry on line

2. **Normal Q-Q.** Quantile-quantile plot. Compare to a normal distribution. The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. 

#want data to follow dashed line
...
#want symmetry around the red line
...
#points outside of dashed lines are outliers

```{r, fig.height = 3, fig.width = 4}
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing plot screen into 4 cells to look at one plot at a time
plot(irradiance.regression)
par(mfrow = c(1,1))
#can remove outliers if really affecting residuals plot
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) #removes outlier from appearing on plot+
  geom_point() 
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #must eliminate 0s to take log and removes outlier
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2)

#runs residuals plots again
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #log transforms axis without changing data set
  geom_point() 
...
#more than one ind x trying to explain variable y
...
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen) #y ~ x + x
summary(TPregression) #runs summary of lm regression
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>% #selects this set of columns to compare
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #correlation function which creates matrix in Environment
corrplot(PeterPaulCorr, method = "ellipse") #creates matrix with ellipse; thinner = more correlation, color shows positive vs. negative correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #shows upper diagonal only and values of correlation
```
...
However, it is possible to over-parameterize a linear model. Adding additional explanatory variables takes away degrees of freedom, and if explanatory variables co-vary the interpretation can become overly complicated. Remember, an ideal statistical model balances simplicity and explanatory power! To help with this tradeoff, we can use the **Akaike's Information Criterion (AIC)** to compute a *stepwise regression that either adds explanatory variables from the bottom up or removes explanatory variables from a full set of suggested options*. The smaller the AIC value, the better. 
  -uses AIC to determine fit between independent & dependent variable
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #aiming to explain total phosphorous concentration by all these x variables

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #removes a variable one by one and runs a regression each time
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)

#best regression based on this analysis considers oxygen, nitrogen, and temperature
