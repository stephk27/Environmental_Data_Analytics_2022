...
Feature is set of coordinates - can create points, lines, or polygons
raster - parses landscape into grid rather than points, lines, or polygones
  image data (pixels)
...
A **coordinate reference system** or **crs** includes the information used to reference the coordinates used in defining a geometry with actual geographic locations. These will be discussed more later. 
  way to map points onto a plane even though the world is spherical
  coordinate transformations: transferring one coordinate system to another
  WGS84 - most popular coordinate system
  UTM - for projected or planar system - longitudinal slices
...
can't read in geographic data with csv because don't store spatial data - can have coordinates
shape files - many files wrapped in GeoJSON
