...
## Objectives
2. Apply special cases of the GLM (ANOVA) to real datasets - analysis of variance (ANOVA)
3. Interpret and report the results of ANOVA in publication-style formats

##Analysis of Variance (ANOVA)
Comparing whether several populations have equal means.
-factor: quantity under examination in experiment as a possible cause of variation
-levels: populations - the measurements or strata of a factor
-balanced design: all factor levels have equal sample sizes

Assumptions:
-all pops are normally distributed
...
**Does total litter biomass differ among sites at the Niwot Ridge LTER? - check if mean of biomass is different due to site location
```{r}
# Wrangle the data
...
# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) #aov - ANOVA function(df, y ~ x)
summary(Litter.Totals.anova)
#H0 for ANOVA is mean is the same across all x 

#results: reject null hypothesis i.e. difference between a pair of group means is statiscally significant; but doesn't tell us which means are not the same

# Format ANOVA as lm function
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
#will have one row for each level of categorical data
...
# Post-hoc test
  #explore difference between multiple group means
# TukeyHSD() computes Tukey Honest Significant Differences used for comparing all possible group pairings
TukeyHSD(Litter.Totals.anova) #function should be run on result of anova test

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #extract groupings for pair relationships
#group = TRUE - groups levels that have the same mean
...
#all observations with the same letter have the same mean
```
...
```{r}
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass) #notice * - multiply to see interaction among to x variables
...
#to look at treatment groups
#creates interaction of functional Group by class
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass))

#Litter.test <- cbind(Litter,Litter.interaction)

#compare means for each level of interaction levels
