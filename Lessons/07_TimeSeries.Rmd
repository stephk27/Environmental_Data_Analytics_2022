...
**For decompose data set, can only use trend data so need to transform data set into time series first.
...
```{r}
#choose column that you want to analyze
...
```{r}
#must decompose each component of the ts
#stl - components are seasonal, trend, and remainder
?stl
...
#object time series will have each stl component
#specify start date but not end date
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3])

#add column with observed data and adding date object
...
**can use interpolation to account for missing data if gaps are not significant
  - will use nearby data to assume data point
...
```{r}
head(wind_data)
summary(wind_data$AWND) #shows how many NAs are in data

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #na.approx function can be used for interpolation

summary(wind_data_clean$AWND.clean)
#Note the NA is gone

ggplot(wind_data_clean ) +
  geom_line(aes(x = DATE, y = AWND.clean), color = "red") + #red line shows interpolated data
...
f_month <- month(first(wind_data_clean$DATE))#defines what the first month is
...
# Run SMK test
#seasonal Mann-Kendall only test that works for seasonal data
#if less than .05 then reject and say that it has a seasonal trend
...
# Inspect results
#will show results for each season of the year and see tendency of trend
wind_data_trend2
