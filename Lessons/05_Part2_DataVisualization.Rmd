...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #base size = font size for labels
  theme(axis.text = element_text(color = "black"), #default axis label text color = gray
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #specifies where is will cross y intercept; lty = line type, 2 = dashed
  geom_hline(yintercept = 100, lty = 2) +
  #good for defining important lines on your plot
  geom_point(alpha = 0.5, size = 1.5) + #alpha = transparency of points
  geom_text(x = as.Date("2020-01-01"), y = 45, #adds specific point & text on the plot where specified
            label = "good", hjust = 1, fontface = "bold") +   # hjust = horizontal justification, 1 means left align
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", #determines how to break axis up
    date_labels = "%b %y") + #how you want your date to be formatted on the graph
  ylab(expression("O"[3]* " AQI Value")) + #[3] is for subscript
  theme(axis.text.x = element_text(angle = 45, #change angle of text so that it doesn't run into each other
                                   hjust = 1))
print(O3plot7)  
...
RColorBrewer (package) - *Super cool color packages*
...
#look at color maps to help you choose color
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, #adds transparency
             size = 2.5) +
  labs(color = "Depth", shape = "Lake Name")+
  #labs() + # change your legend labels here
  scale_shape_manual(values = c(15, 17)) + #change shape of point - one for each variable
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) + #direction - which direction for color to start appearing
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(y = "Concentration", x = "Lake Name", color = "Nutrient")
...
#Cowplot allows you to define as many plots as you want on the same page
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, #says to plot one below the other
          align = 'h', 
...
ggplot(Neonics) +
  geom_bar(aes(x = Endpoint), color = "darkred", fill = "lightgray")+
  scale_x_discrete(limits = rev(levels(Neonics$Endpoint)))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(x = "End Point", y = "Count")

#I couldn't figure out how to do the reorder thing
