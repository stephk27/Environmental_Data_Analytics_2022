...
#### Define UI ----
ui <- fluidPage(#theme = shinytheme("united"), #the themes would not run for me
  titlePanel("Lake Nutrients"),
  sidebarLayout(
    sidebarPanel(
...
#1. Play with changing the options on the sidebar. 
    # Choose a shinytheme that you like. The default here is "yeti"
        # I tried changing it to "united", but R shiny wouldn't run on my computer with a theme.
...
#2. How is the mainPanel component of the UI structured? 
    # How does the output appear based on this code?
        #The mainPanel is structured to show the scatterplot, which is then designed with ggplot with the server function.


#3. Explore the reactive formatting within the server.
    # Which variables need to have reactive formatting? 
      #Sampledate, depth, and lake name all  need to have reactive formatting.
      #These are the variables that can be changed by the user, but are not the primary variable being displayed.

    # How does this relate to selecting rows vs. columns from the original data frame?
      #This feature is similar to filtering data in a dataframe.
