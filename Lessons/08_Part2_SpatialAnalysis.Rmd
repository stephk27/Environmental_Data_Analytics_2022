...
- Spatial aggregation with `group_by` and `summarize` or `st_union` 
    can group by spatial features (eg. county)
    spatial features dissolve (merge together)
...
#read in spatial data sets (shape file = GeoJSON)
...
head(huc8_sf_challenge)


#Create a map
ggplot() +
  geom_sf(data = huc8_sf, aes(fill = 'orange')) +
  geom_sf(data = huc8_sf_challenge, aes(fill = 'purple'))
...
huc8_nc_raw <- read.csv(
  ...
  group_by(HUC_8) %>%
  summarise(WindEnergy = sum(AvgWindEnergy))



#Join to HUC_8 features
huc8_join <- merge(huc8_sf, huc8_nc_raw, by.x = "HUC_8", by.y = "HUC_8")



#View the outputs
mapview(huc8_join, zcol = "WindEnergy") #OR

ggplot(huc8_join) + 
  geom_sf(aes(fill = WindEnergy)) + 
