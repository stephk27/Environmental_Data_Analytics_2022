...
#read.table part of base R, skip = 28 is to skip lines before the data, sep = how the data is separated
#can easily count header lines by pasting into notepad
...
APIs provide data in a more readable format: csv or json usually

YOUR_KEY_GOES_HERE = 
need to sign up for a key - look up key for whatever database you are using
  allows database to keep track of who is using (and maybe abusing) server


### Grabbing discharge data via  the `dataRetrieval` package
automate downloading from servers in tidy format

```{r using.the.dataRetrieval.package}
#better to define variables for what you want to pull so that you can more easily reuse code
...
neuse <- readNWISdv(siteNumbers = siteNo, 
                    ...
                    endDate=end.date)
#look up function to see what values you need to provide
...
ggplot(neuse_by_year, aes(x=year,y=mean_annual_flow)) + 
  geom_line() +
  labs(x = "Year", y="Mean Annual Flow (cfs)")
```
...
You can store your API key somewhere.  I'm not sure where. Ask John. Maybe below code.
...
#creates graph of counties in NC and maps population
nc_pop <- 
...
#use selector gadget to select the data you want to scrape
  #need to hold control while you select elements that you want
...
#as.numeric doesn't work with data with commas - use gsub to remove commas first
the_df <- data.frame(
  "States" = the_states,
  "Price" = as.numeric(the_price),
  "Capacity" = as.numeric(gsub(",","",the_capacity)),
  "Generation" = as.numeric(gsub(",","",net_generation)),
  "Retail_Value" = as.numeric(gsub(",","",total_retail))
)
